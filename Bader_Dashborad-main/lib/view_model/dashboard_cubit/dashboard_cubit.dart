...
import 'package:bader_dashboard/shared/Constants/enumeration.dart';
...
class DashBoardCubit extends Cubit<DashBoardStates> {
  ...
  static DashBoardCubit getInstance(BuildContext context) =>
      BlocProvider.of(context);

  // TODO : SEND A NOTIFICATION TO USER AFTER MAKE HIM A LEADER ON SPECIFIC CLUB
  Future<bool> sendNotifyToUserAfterMakingHimALeaderOnSpecificClub(
    ...
    try {
      await FirebaseFirestore.instance
          ...
          .doc(receiverID)
          .collection(Constants.kNotificationsCollectionName)
          .add(model.toJson());
      return true;
  ...
  List<CategoryModel> adminCategories = [
    ...
    CategoryModel(
        ...
        routeName: "assign_Club_Leader"),
    CategoryModel(
        title: "عرض التقارير",
        iconData: Icons.display_settings,
        routeName: "view_Reports"),
    CategoryModel(
    ...
    CategoryModel(
        title: "حذف نادي", iconData: Icons.clear, routeName: "delete_Club"),
  ];
  ...
  Future<void> getClubsWithoutLeader() async {
    ...
    debugPrint(
        "Clubs that without leader number is : ${clubsWithoutLeaderData.length}");
  ...
  Future<ClubModel> getInfoForClubChosenFromDropDownButton(
      {required String clubName}) async {
    return clubs
        .firstWhere((element) => element.name!.trim() == clubName.trim());
  }

  // Todo: Responsible to change password by click on Forget Password Button
  Future<void> createClub(
      {required String name, required String college}) async {
  ...
  Future<void> assignClubLeader(
      ...
      required String leaderEmail}) async {
    emit(AssignLeaderToClubLoadingState());
    try {
      await dashboardRepository.assignClubLeader(
          clubID: clubID,
          leaderID: leaderID,
      ...
      bool sendNotification =
          await sendNotifyToUserAfterMakingHimALeaderOnSpecificClub(
              clubID: clubID, clubName: clubName, receiverID: leaderID);
  ...
  Future<void> acceptOrRejectPlanForClub(
    ...
    try {
      await dashboardRepository.acceptOrRejectPlanForClub(
          report: report, responseStatus: responseStatus);
      await getAllReports();
      emit(AcceptOrRejectPlanForClubSuccessState());
    } on FirebaseException catch (e) {
      emit(
          FailedToAcceptOrRejectPlanForClubState(message: e.message ?? e.code));
    }
  }
