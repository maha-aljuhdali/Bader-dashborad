...
class DashboardRepository {
  ...
  Future<void> createClub(
    ...
    ClubModel model =
        ClubModel(name: name, id: lastIdForClubCreated, college: college);
    await FirebaseFirestore.instance
        .collection(Constants.kClubsCollectionName)
        .doc(lastIdForClubCreated.toString())
        .set(model.toJson());
  }
  ...
  Future<void> assignClubLeader(
      ...
      required String leaderName}) async {
  ...
  Future<bool> sendNotification(
      {required String receiverID, required NotifyModel notifyModel}) async {
    try {
      await FirebaseFirestore.instance
          .collection(Constants.kUsersCollectionName)
          .doc(receiverID)
          .collection(Constants.kNotificationsCollectionName)
          .add(notifyModel.toJson());
      return true;
  ...
  Future<void> acceptOrRejectPlanForClub(
    ...
    NotifyModel notifyModel = NotifyModel(
        ...
        notifyType: responseStatus
            ? NotificationType.acceptPlanForClubYouLead.name
